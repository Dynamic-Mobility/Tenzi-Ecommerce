...
export default async function handler(req, res) {
    if (req.method === API_METHODS.GET) {
        try {
            ...
            await backendAxiosInstance.get(`${API_URL.FETCH_STORE_MENU}`, config).then(response => {
        ...
        try {
            ...
            console.log("MENU_URL ",`${API_URL.CREATE_STORE_MENU}`);

            await backendAxiosInstance.post(`${API_URL.CREATE_STORE_MENU}`,body,config)
                .then(response => {
                    res.status(200).json(response.data);
                })
                .catch(e => {
